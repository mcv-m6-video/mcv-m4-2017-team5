...
images_train = zeros(dim1, dim2, dim3,length(train));
images_test = zeros(dim1, dim2, dim3, length(test));
...
for i = train
    images_train(:,:,:,i) = im2double(imread(dirIn(i).name));
end
...
for m = 1:dim1
    for n = 1:dim2
        for k = 1:dim3
            mu(m,n,k)=mean(images_train(m,n,k,:));
            sigma(m,n,k)=std(images_train(m,n,k,:)); 
        end
    end
end
...
for i = test
    images_test(:,:,:,i-length(test)+1) = im2double(imread(dirIn(i).name));
    imagesSeg(:,:,i-length(test)+1) = ...
        abs(images_test(:,:,1,i-length(test)+1)-mu(:,:,1)) >= alpha*(2+sigma(:,:,1)) &...
        abs(images_test(:,:,2,i-length(test)+1)-mu(:,:,2)) >= alpha*(2+sigma(:,:,2)) &...
        abs(images_test(:,:,3,i-length(test)+1)-mu(:,:,3)) >= alpha*(2+sigma(:,:,3));
end
